...
public struct FirstMatchSpec<Context, Result>: DecisionSpec {
    ...
    // Explicit label avoids tuple overload ambiguity in Swift 6.4's builder call.
    private init(erasedPairs: [SpecificationPair], includeMetadata: Bool) {
        self.pairs = erasedPairs
        self.includeMetadata = includeMetadata
    }
...
public extension FirstMatchSpec {
    ...
    class Builder<C, R> {
        ...
        public func build() -> FirstMatchSpec<C, R> {
            FirstMatchSpec<C, R>(
                erasedPairs: pairs.map { (specification: $0.0, result: $0.1) }, includeMetadata: includeMetadata
            )
        }
    }
